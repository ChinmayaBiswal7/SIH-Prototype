...
export default function MapView({ intersections, onSelectIntersection, corridor, onCancelCorridor }) {
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
      ...
      BBSR_ROAD_SEGMENTS.forEach((seg) => {
        ...
        L.polyline(pts, {
          color: isHighway ? "rgba(56, 189, 248, 0.42)" : isStateHighway ? "rgba(99, 102, 241, 0.32)" : "rgba(148, 163, 184, 0.22)",
  ...
  useEffect(() => {
    ...
    BBSR_INTERSECTIONS.forEach((junc) => {
      ...
      const zoneColor = ZONE_COLORS[junc.zone] || "#38bdf8";
      ...
      const tooltipHtml = `
        <div class="traffic-tooltip-content">
          ...
          <div class="tt-row">
            ...
            <strong style="color: ${isCorridorNode ? '#10b981' : isCritical ? '#ef4444' : isMedium ? '#f59e0b' : '#10b981'}">
  ...
  useEffect(() => {
    ...
    if (tracedVehicle && tracedRouteGeometry.length > 0) {
      ...
      L.polyline(tracedRouteGeometry, {
        color: tracedVehicle.color || "#10b981",
        weight: 10,
        opacity: 0.35,
        lineCap: "round",
        lineJoin: "round",
  ...
  useEffect(() => {
    ...
    if (corridor && corridor.isActive && roadGeometry.length > 1) {
      ...
      L.marker(pStart, {
        icon: L.divIcon({
          className: "route-waypoint-icon",
          html: `<div class="route-origin-pin" style="background:#059669;border:1.5px solid #34d399;box-shadow:0 0 12px rgba(16,185,129,0.7)">🟢 DISPATCH: ${(corridor.origin || "ORIGIN").split(" ")[0]}</div>`,
        }),
        zIndexOffset: 12000,
      }).addTo(group);
      ...
      L.marker(pEnd, {
        icon: L.divIcon({
          className: "route-waypoint-icon",
          html: `<div class="route-dest-pin" style="background:#dc2626;border:1.5px solid #f87171;box-shadow:0 0 12px rgba(239,68,68,0.7)">🏥 DEST: ${(corridor.destination || "DEST").split(" ")[0]}</div>`,
        }),
        zIndexOffset: 12000,
      }).addTo(group);
        ...
        <div className="map-badge-card">
          <div className="map-legend">
            ...
            <span className="legend-item" style={{ marginLeft: "8px" }}>
              <span style={{ color: "#38bdf8" }}>━━</span> NH-16
            </span>
            <span className="legend-item">
              <span style={{ color: "#818cf8" }}>━━</span> State Arterials
            </span>
          </div>
        </div>
      </div>
                ...
                return (
                  ...
                  >
                        ...
                        <div className="unit-card-callsign">
                          ...
                          <span
                            ...
                            style={{
                              ...
                              color: isArrived ? '#10b981' : unit.color,
                              borderColor: isArrived ? '#10b98155' : `${unit.color}55`
                            }}
                          >
                            <span className="live-status-dot" style={{ background: isArrived ? '#10b981' : unit.color }} />
                      ...
                      <div className="unit-prog-track">
                        <div
                          ...
                          style={{
                            ...
                            background: isArrived ? '#10b981' : `linear-gradient(90deg, ${unit.color}88, ${unit.color})`,
                            boxShadow: `0 0 10px ${isArrived ? '#10b981' : unit.color}`
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="unit-card-footer">
                      ...
                      <div className="unit-tele-live" style={{ color: isArrived ? '#10b981' : unit.color }}>
                        <span className="live-blink-dot" style={{ background: isArrived ? '#10b981' : unit.color }} />
        ...
        <div className="traced-vehicle-card">
          ...
          <div className="tvc-stats-grid">
            ...
            <div className="tvc-stat">
              ...
              <strong className="tvc-stat-val" style={{ color: "#10b981" }}>READY</strong>
            </div>
          </div>
          ...
          <div className="jpc-header">
            ...
            <div>
              ...
              <span
                ...
                style={{
                  background: `${ZONE_COLORS[selectedJunction.zone] || '#38bdf8'}22`,
                  color: ZONE_COLORS[selectedJunction.zone] || '#38bdf8',
                  border: `1px solid ${ZONE_COLORS[selectedJunction.zone] || '#38bdf8'}66`
                }}
              >
          ...
          {corridor.progress >= 100 ? (
            <div className="corridor-arrived-banner">
              <div className="arrived-icon-wrap">
                <i className="fas fa-check-circle" style={{ fontSize: "1.4rem", color: "#10b981" }} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: "0.85rem", color: "#f8fafc", letterSpacing: "0.3px" }}>
            ...
            <>
              ...
              <div className="hud-bottom-row">
                <div className="hud-gps-tag">
                  <i className="fas fa-crosshairs" style={{ color: "#38bdf8" }} />
